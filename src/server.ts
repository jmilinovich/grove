...
// "collection" keyed by the vault's on-disk basename. Search calls in this
// process MUST filter by this collection — without it, a query against
// vault A's MCP server returns notes from every other vault on the box.
...
Use the content_hash as if_hash when updating the note.`,
    ...
    async ({ file }) => {
      ...
      // Scope to this vault's collection — defense-in-depth alongside the
      // existsSync check below, and avoids picking a foreign-vault path
      // as the top ranker when this vault has no match.
      try {
